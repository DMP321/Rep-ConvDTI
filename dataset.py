...
def collate_fn(batch_data):
    ...
    compound_max = 100
    protein_max = 1000
    onehot_compound = torch.zeros((N, compound_max),dtype=torch.long)
    onehot_protein = torch.zeros((N, protein_max),dtype=torch.long)
    ...
    for i,pair in enumerate(batch_data):
        ...
        onehot_compound[i] = compoundint
        proteinint = torch.from_numpy(label_sequence(proteinstr, CHARPROTSET,protein_max))
        onehot_protein[i] = proteinint
        label = float(label)
        labels[i] = label
    return (onehot_compound, onehot_protein, labels)
